...
public class Pocolo extends Boss {
    ...
    public Pocolo(Zone zone, Player player, long dame, long hp) throws Exception {
        super(PHOBAN, BossID.POCOLO, new BossData(
                ...
                new int[][]{
                    {Skill.DEMON, 7, 1000},
                    {Skill.MASENKO, 1, 100000}, {Skill.MASENKO, 2, 100000}, {Skill.MASENKO, 3, 100000}, {Skill.MASENKO, 4, 100000}, {Skill.MASENKO, 5, 100000}, {Skill.MASENKO, 6, 100000}, {Skill.MASENKO, 7, 100000},
                    {Skill.KAMEJOKO, 1, 100000}, {Skill.KAMEJOKO, 2, 100000}, {Skill.KAMEJOKO, 3, 100000}, {Skill.KAMEJOKO, 4, 100000}, {Skill.KAMEJOKO, 5, 100000}, {Skill.KAMEJOKO, 6, 100000}, {Skill.KAMEJOKO, 7, 100000},
                    {Skill.ANTOMIC, 1, 100000}, {Skill.ANTOMIC, 2, 100000}, {Skill.ANTOMIC, 3, 100000}, {Skill.ANTOMIC, 4, 100000}, {Skill.ANTOMIC, 5, 100000}, {Skill.ANTOMIC, 6, 100000}, {Skill.ANTOMIC, 7, 100000},
                    {Skill.TAI_TAO_NANG_LUONG, 1, 150000},
                },
    ...
    public synchronized double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            ...
            if (damage >= this.nPoint.hp && !this.isLaze) {
                this.laze();
                return 0;
            }
    ...
    public void autoLeaveMap() {
        ...
        if (this.isLaze && !playerCall.winSTT && Util.canDoWithTime(lastTimeLaze, 3000)) {
            ...
            try {
                msg = new Message(-60);
                msg.writer().writeInt((int) this.id);
                msg.writer().writeByte(83);
                msg.writer().writeByte(1);
                msg.writer().writeInt((int) playerCall.id);
                msg.writer().writeByte(1);
                msg.writer().writeByte(1);
                msg.writer().writeInt(1_000_000_000);
                msg.writer().writeBoolean(true);
                msg.writer().writeBoolean(this.nPoint.isCrit);
                Service.gI().sendMessAllPlayerInMap(this, msg);
                msg.cleanup();
            } catch (IOException e) {
            }
            ...
            final Player playerKilled = this.playerCall;
            new Thread(() -> {
                try {
                    Thread.sleep(3500);
                    if (playerKilled != null && playerKilled.zone != null && MapService.gI().isMapSieuThanhThuy(playerKilled.zone.map.mapId)) {
                        Player mc = playerKilled.zone.getNpc();
                        if (mc != null) {
                            int x = playerKilled.location.x + Util.nextInt(-50, 50);
                            int y = 336;
                            PlayerService.gI().playerMove(mc, x, y);
                        }
                        if (playerKilled.isDie()) {
                            Service.gI().hsChar(playerKilled, playerKilled.nPoint.hpMax, playerKilled.nPoint.hpMax);
                        }
                        NpcService.gI().createMenuConMeo(playerKilled, ConstNpc.MENU_SIEU_THAN_THUY, 2119, "Để tôi đưa cậu về", "Đồng ý", "Từ chối");
                        playerKilled.lastTimeUpdateSTT = System.currentTimeMillis();
                    }
