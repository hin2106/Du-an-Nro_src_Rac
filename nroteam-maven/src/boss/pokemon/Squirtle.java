...
public class Squirtle extends Boss {

    private long st;

    public Squirtle() throws Exception {
        super(BossID.SQUIRTLE, BossesData.SQUIRTLE);
    }
                        ...
                        new Thread(() -> {
                            try {
                                ...
                                if (InventoryService.gI().getCountEmptyBag(plKill) > 0) {
                                    Item reward = ItemService.gI().createNewItem((short) 1804);
                                    reward.quantity = 1;
                                    try {
