...
public class Bullbasaur extends Boss {

    private long st;

    public Bullbasaur() throws Exception {
        super(BossID.BULLBASAUR, BossesData.BULLBASAUR);
    }
                        ...
                        new Thread(() -> {
                            try {
                                ...
                                if (InventoryService.gI().getCountEmptyBag(plKill) > 0) {
                                    Item reward = ItemService.gI().createNewItem((short) 1805);
                                    reward.quantity = 1;
                                    try {
