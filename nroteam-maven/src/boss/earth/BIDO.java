...
public class BIDO extends Boss {
    ...
    public void reward(Player plKill) {
        Service.gI().dropItemMap(this.zone, new ItemMap(zone, 190, Util.nextInt(1, 10), this.location.x + Util.nextInt(-50, 50), this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id));
        ...
        for (int i = 0; i < Util.nextInt(3, 10); i++) {
            Service.gI().dropItemMap(this.zone, new ItemMap(zone, 190, Util.nextInt(1_000_000, 20_000_000), this.location.x + i * 10, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 24), plKill.id));
        }
        for (int i = 1; i < Util.nextInt(3, 10) + 1; i++) {
            Service.gI().dropItemMap(this.zone, new ItemMap(zone, 190, Util.nextInt(1_000_000, 20_000_000), this.location.x - i * 10, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id));
        }
        short itTemp = 426;
