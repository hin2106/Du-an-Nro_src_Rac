...
public class SAIBAMEN extends Boss {
    ...
    public void afk() {
        ...
        long alive = this.clan.ConDuongRanDoc.getNumBossAlive();
        if (this.idboss == 1) {
    ...
    public void leaveMap() {
        ...
        if (this.clan != null && this.clan.ConDuongRanDoc != null) {
            if (this.isDie()) {
                if (this.idboss < 6) {
                    for (Boss b : this.clan.ConDuongRanDoc.bosses) {
                        if (b instanceof SAIBAMEN sai && sai.idboss == this.idboss + 1) {
                            if (b.bossStatus == BossStatus.AFK) {
                                sai.changeToTypePK();
                                sai.changeStatus(BossStatus.ACTIVE);
                            }
                            break;
                        }
                    }
                } else if (this.idboss == 6) {
                    // Wake up NADIC when Sai 6 disappears
                    for (Boss b : this.clan.ConDuongRanDoc.bosses) {
                        if (b.id == BossID.NADIC && b.bossStatus == BossStatus.AFK) {
                            b.changeToTypePK();
