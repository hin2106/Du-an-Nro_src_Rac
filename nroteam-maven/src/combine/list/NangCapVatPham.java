...
public class NangCapVatPham {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 4) {
            ...
            if (CombineSystem.isCoupleItemNangCapCheck(itemDo, itemDNC) && itemDo != null && itemDNC != null) {
                if (itemDo.itemOptions == null) {
                    CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Trang bị không hợp lệ",
                            "Đóng");
                    return;
                }
                int level = 0;
                ...
                if (level < CombineService.MAX_LEVEL_ITEM) {
                    ...
                    String optionText = "";
                    if (option != null) {
                        optionText = option.replaceAll("#", String.valueOf(param)) + "\n";
                    }
                    ...
                    if (player.combineNew.countDaNangCap > itemDNC.quantity) {
                        ...
                        CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                ...
                                "Còn thiếu\n"
                                        ...
                                        + " vàng");
                    } else if (player.combineNew.itemsCombine.size() == 3 && itemDBV != null
    ...
    public static void nangCapVatPham(Player player) {
        if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 4) {
            ...
            if (CombineSystem.isCoupleItemNangCapCheck(itemDo, itemDNC) && itemDo != null && itemDNC != null) {
                if (itemDo.itemOptions == null) {
                    Service.gI().sendThongBao(player, "Trang bị không hợp lệ");
                    return;
                }
                ...
                if (level < MAX_LEVEL_ITEM) {
                    ...
                    if (option == null) {
                        Service.gI().sendThongBao(player, "Trang bị không có chỉ số phù hợp để nâng cấp");
                        player.inventory.gold += gold;
                        return;
                    }
                    if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                        ...
                        dragonpass.DragonPassService.gI().onEquipmentUpgraded(player);
                        if (level == 7) {
