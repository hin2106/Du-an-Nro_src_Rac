...
public final class WorldMartialArtsTournament implements Runnable {
    ...
    // Item ID for Thúng Nếp (event Tết)
    private static final int ITEM_THUNG_NEP = 749;
    private static final int NEP_WIN_QUANTITY = 5;
    private static final int NEP_LOSE_QUANTITY = 1;
        ...
        try {
            ...
            if (plWin != null) {        
                if (WorldMartialArtsTournamentManager.gI().round == 1) {
                    ...
                    dragonpass.DragonPassService.gI().onTournamentRoundTwoWon(plWin);
                }
            ...
            if (plWin != null) {
                ...
                KolTaskService.doneTaskKolMartialArtsTournament(plWin);
                
                // Event Tết: Thưởng Thúng Nếp cho người thắng
                if (Manager.EVENT_SEVER == consts.ConstEvent.SU_KIEN_TET) {
                    giveNepReward(plWin, NEP_WIN_QUANTITY);
                }
            }
            
            // Event Tết: Thưởng Thúng Nếp cho người thua (ít hơn)
            if (plLose != null && Manager.EVENT_SEVER == consts.ConstEvent.SU_KIEN_TET) {
                giveNepReward(plLose, NEP_LOSE_QUANTITY);
            }
            
            // Thưởng lá bùa ngẫu nhiên (537-540) cho cả 2 người, miễn thắng thua
            if (plWin != null) giveRandomCharm(plWin);
            if (plLose != null) giveRandomCharm(plLose);
    ...
    /**
     * Thưởng x1 lá bùa ngẫu nhiên (537-540) cho người chơi
     */
    private void giveRandomCharm(Player pl) {
        try {
            if (pl == null || pl.zone == null) return;
            int charmId = Util.nextInt(537, 540);
            Item charm = ItemService.gI().createNewItem((short) charmId);
            InventoryService.gI().addItemBag(pl, charm);
            InventoryService.gI().sendItemBags(pl);
            Service.gI().sendThongBao(pl, "Bạn nhận được 1 " + charm.template.name + "!");
        } catch (Exception e) {
        }
    }

    /**
     * Give Thúng Nếp reward during Tết event
     */
    private void giveNepReward(Player pl, int quantity) {
        try {
            if (pl == null || pl.zone == null) return;
            
            Item nep = ItemService.gI().createNewItem((short) ITEM_THUNG_NEP, quantity);
            InventoryService.gI().addItemBag(pl, nep);
            InventoryService.gI().sendItemBags(pl);
            Service.gI().sendThongBao(pl, "Bạn nhận được " + quantity + " Thúng Nếp!");
        } catch (Exception e) {
            // Ignore errors
        }
    }
