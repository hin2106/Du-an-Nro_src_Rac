...
public class RedRibbonHQ implements Runnable {
    ...
    private ItemMap createNRById(Zone zone, int itemId) {
        int x = Util.nextInt(100, zone.map.mapWidth - 100);
        int y = zone.map.yPhysicInTop(x, 100);
        return new ItemMap(zone, itemId, 1, x, y, -1);
    }
    ...
    public void randomNR() {
        ...
        Zone zone = zones.get(Util.nextInt(0, zones.size() - 1));
        Service.gI().dropItemMap(zone, createNRById(zone, 17));
    }
    ...
    public synchronized void clearMobsAfterVictory() {
        for (Zone zone : zones) {
            zone.isTUTAlive = false;
            zone.isbulon1Alive = false;
            zone.isbulon2Alive = false;
            for (Mob mob : zone.mobs) {
            ...
            for (Player boss : zone.getBosses()) {
                if (boss instanceof Boss b && !b.isDie()) {
                    b.leaveMap();
                }
            }
        }
