...
public class MessageSendCollect implements IMessageSendCollect {
    ...
    public Message readMessage(ISession session, DataInputStream dis) throws Exception {
        ...
        if (size > AntiDDoSEngine.gI().getMaxInboundPacketSize()) {
            throw new IOException("Inbound packet too large: " + size);
        }
        ...
        if (session.sentKey()) {
            ...
            return new Message(cmd, data, size);
    }
    ...
    public void doSendMessage(ISession session, DataOutputStream dos, Message msg) throws Exception {
        try {
            ...
            if (data != null) {
                ...
                if (msg.command == -32 || msg.command == -66 
                        || msg.command == -74 || msg.command == 11
                        || msg.command == -67 || msg.command == -87 
                        || msg.command == 66 || msg.command == 12) {
