...
public class TaskService {
    ...
    public void sendNextTaskMain(Player player) {
        ...
        services.top.TopNhiemVuService.gI().invalidateCache();
        ...
        // Cập nhật lastTime cho nhiệm vụ mới
        player.playerTask.taskMain.lastTime = System.currentTimeMillis();
    ...
    public void checkDoneTaskGetItemManhGiayMa(Player player) {
        if (!player.isBoss && !player.isPet) {
            doneTask(player, ConstTask.TASK_19_6);
        }
    }

    public void checkDoneTaskJoinMapTayKarin(Player player) {
        if (!player.isBoss && !player.isPet) {
            doneTask(player, ConstTask.TASK_19_7);
        }
    }

    public void checkDoneTaskKillMobTayKarin(Player player) {
        if (!player.isBoss && !player.isPet) {
            doneTask(player, ConstTask.TASK_19_8);
        }
    }
    ...
    public void spawnBabyItemForPlayer(Player player) {
        if (player == null || player.zone == null) {
            return;
        }
        int mapId = player.zone.map.mapId;
        if ((mapId >= 42 && mapId <= 44) && getIdTask(player) == ConstTask.TASK_3_1) {
            boolean alreadyHasBaby = false;
            for (ItemMap it : player.zone.items) {
                if (it.itemTemplate.id == 78 && it.playerId == player.id) {
                    alreadyHasBaby = true;
                    break;
                }
            }
            if (!alreadyHasBaby) {
                int x = 500 + utils.Util.nextInt(-100, 100);
                int y = player.zone.map.yPhysicInTop(x, 300);
                ItemMap babyItem = new ItemMap(player.zone, 78, 1, x, y, player.id);
                Service.gI().dropItemMap(player.zone, babyItem);
            }
        }
    }

    public void removeBabyItemForPlayer(Player player) {
        if (player == null || player.zone == null) {
            return;
        }
        try {
            for (int i = player.zone.items.size() - 1; i >= 0; i--) {
                ItemMap it = player.zone.items.get(i);
                if (it != null && it.itemTemplate != null && it.itemTemplate.id == 78 && it.playerId == player.id) {
                    services.map.ItemMapService.gI().removeItemMapAndSendClient(it);
                }
            }
        } catch (Exception e) {
            Logger.logException(TaskService.class, e, "Error removing baby item");
        }
    }
        ...
        if (!player.isBoss && !player.isPet && item != null && item.itemTemplate != null) {
            switch (item.itemTemplate.id) {
                ...
                case 78: // em bé
                    ...
                    removeBabyItemForPlayer(player);
                    break;
                case 20:
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet) {
            switch ((int) boss.id) {
                case BossID.TAUPAYPAY -> doneTask(player, ConstTask.TASK_10_1);
                case BossID.KARIN -> doneTask(player, ConstTask.TASK_10_0);
                case BossID.KUKU -> doneTask(player, ConstTask.TASK_21_0);
                case BossID.MAP_DAU_DINH -> doneTask(player, ConstTask.TASK_21_1);
                case BossID.RAMBO -> doneTask(player, ConstTask.TASK_21_2);
                case BossID.SO_4 -> doneTask(player, ConstTask.TASK_22_0);
                case BossID.SO_3 -> doneTask(player, ConstTask.TASK_22_1);
                case BossID.SO_1 -> doneTask(player, ConstTask.TASK_22_2);
                case BossID.TIEU_DOI_TRUONG -> doneTask(player, ConstTask.TASK_22_3);
                case BossID.FIDE -> {
                ...
                case BossID.ANDROID_19 -> doneTask(player, ConstTask.TASK_25_1);
                case BossID.DR_KORE -> doneTask(player, ConstTask.TASK_25_2);
                case BossID.ANDROID_15 -> doneTask(player, ConstTask.TASK_26_1);
                case BossID.ANDROID_14 -> doneTask(player, ConstTask.TASK_26_2);
                case BossID.ANDROID_13 -> doneTask(player, ConstTask.TASK_26_3);
                case BossID.POC -> doneTask(player, ConstTask.TASK_27_2);
                case BossID.PIC -> doneTask(player, ConstTask.TASK_27_1);
                case BossID.KING_KONG -> doneTask(player, ConstTask.TASK_27_3);
                ...
                case BossID.SIEU_BO_HUNG -> doneTask(player, ConstTask.TASK_29_4);
                ...
                case BossID.YA_CON -> doneTask(player, ConstTask.TASK_30_4);
                case BossID.MABU_12H -> doneTask(player, ConstTask.TASK_30_6);
                case BossID.TRUNG_UY_TRANG -> doneTask(player, ConstTask.TASK_19_1);
            }
        }
    }
    ...
    public void checkDoneTaskKillMob(Player player, Mob mob) {
        ...
        if (!player.isBoss && !player.isPet) {
            dragonpass.DragonPassService.gI().onMobKilled(player, mob.tempId);
            if (mob.tempId == ConstMob.MOC_NHAN) {
                try {
                    task.KolTaskService.doneTaskKolKillMob(player);
                } catch (Exception e) {
                    utils.Logger.log("Error updating KOL task for Moc Nhan: " + e.getMessage());
                    e.printStackTrace();
                }
            }
            try {
                task.KolTaskService.doneTaskKolKillAnyMob(player);
            } catch (Exception e) {
                utils.Logger.log("Error updating KOL task for kill mob: " + e.getMessage());
                e.printStackTrace();
            }
    ...
    public boolean doneTask(Player player, int idTaskCustom) {
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                // npcSay(player, ConstNpc.BO_MONG, "Xem ra cậu đã mạnh hơn lần trước tôi gặp
                // rồi đó cậu có muốn thách đấu tại tên Tàu Pảy Pảy không?");
    ...
    private void npcSay(Player player, int npcId, String text) {
        ...
        NpcService.gI().createTutorial(player, npcId, avatar, text);
    }
    ...
    private void rewardDoneTask(Player player) {
        switch (player.playerTask.taskMain.id) {
            ...
            case 11:
                ...
                item.quantity = 10;
                // Kiểm tra hành trang trước khi thêm item
                if (InventoryService.gI().getCountEmptyBag(player) >= 1) {
                    if (InventoryService.gI().addItemBag(player, item)) {
                        InventoryService.gI().sendItemBags(player);
                        Service.gI().sendThongBao(player, "Bạn đã nhận được phần thưởng nhiệm vụ!");
                    } else {
                        Service.gI().sendThongBao(player, "Không thể thêm vật phẩm thưởng vào hành trang.");
                    }
                } else {
                    Service.gI().sendThongBao(player, "Hành trang không đủ chỗ trống để nhận thưởng!");
                }
                break;
            case 13:
                itemId = 85;
                ...
                break;
            case 33:
                // Task 33 không có phần thưởng vật phẩm
                break;
        }
        ...
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                switch (player.playerTask.sideTask.level) {
                    case ConstTask.EASY -> goldReward = ConstTask.GOLD_EASY;
                    case ConstTask.NORMAL -> goldReward = ConstTask.GOLD_NORMAL;
                    case ConstTask.HARD -> goldReward = ConstTask.GOLD_HARD;
                ...
                if (player.mabuEgg != null) {
                    switch (player.playerTask.sideTask.level) {
                        case ConstTask.EASY -> player.mabuEgg.subTimeDone(0, 0, 30, 0);
                        case ConstTask.NORMAL -> player.mabuEgg.subTimeDone(0, 1, 0, 0);
                        case ConstTask.HARD -> player.mabuEgg.subTimeDone(0, 2, 0, 0);
                        case ConstTask.VERY_HARD -> player.mabuEgg.subTimeDone(0, 4, 30, 0);
                        case ConstTask.HELL -> player.mabuEgg.subTimeDone(0, 2, 0, 0);
                    }
                } // nếu không có mabuEgg thì bỏ qua, không log lỗi

                player.inventory.addGold(goldReward);
                Service.gI().sendMoney(player);
                Service.gI().sendThongBao(player, "Bạn nhận được " + Util.numberToMoney(goldReward) + " vàng");
                if (player.playerTask.sideTask.level == ConstTask.VERY_HARD
                        && player.playerTask.sideTask.isDone()) {
                    try {
                        task.KolTaskService.doneTaskKolBoMong(player);
                    } catch (Exception e) {
                        utils.Logger.log("Error updating KOL task for Very Hard side task: " + e.getMessage());
                    }
                }

                // Halloween bonus: Pumpkin star based on difficulty
                try {
                    if (server.Manager.EVENT_SEVER == consts.ConstEvent.SU_KIEN_HALLOWEEN) {
                        event.halloween.BoMongHalloween.reward(player, (byte) player.playerTask.sideTask.level);
                    }
                } catch (Exception ignored) {
                }
                
                // Event Tết: Thưởng Đậu Xanh khi hoàn thành nhiệm vụ Bò Mộng
                try {
                    if (server.Manager.EVENT_SEVER == consts.ConstEvent.SU_KIEN_TET) {
                        int dauXanhQuantity = Util.nextInt(1, 15);
                        Item dauXanh = ItemService.gI().createNewItem((short) 750, dauXanhQuantity);
                        InventoryService.gI().addItemBag(player, dauXanh);
                        Service.gI().sendThongBao(player, "Bạn nhận được " + dauXanhQuantity + " Thúng Đậu Xanh!");
                    }
                } catch (Exception ignored) {
                }
