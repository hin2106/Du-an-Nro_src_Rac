...
public class SkillService {
    ...
    // Bug skill 9 - Dùng skill 9 xong đổi nhanh chiêu đấm sẽ gây dame lớn.
    ...
    public void newSkillNotFocus(Player player, int status) {
        ...
        try {
            ...
            if (status == 20) {
                ...
                boolean isFly = false;
                // Resolve per-player overrides from NewSkill getters (books by planet)
                byte typePaint = newSkill.getTypePaint();
                byte typeItem = newSkill.getTypeItem();
                // persist into newSkill so status 21 uses the same values
                newSkill.typePaint = typePaint;
                newSkill.typeItem = typeItem;
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN:
                ...
                if (player.setClothes.thanVuTruKaio == 4) {
                    hpUse = player.nPoint.hpMax / 100 * 7;
                ...
                Service.gI().sendEffAllPlayer(player, 1027, 0, 5, 10);
            ...
            case Skill.MASENKO:
            case Skill.ANTOMIC:
                EffectSkillService.gI().sendEffectUseSkill(player, player.playerSkill.skillSelect.template.id);
                if (plTarget != null) {
            ...
            case Skill.THOI_MIEN:
                ...
                if (plTarget != null) {
                    ...
                    int timeSeconds = timeSleep / 1000;
                    ItemTimeService.gI().sendItemTime(plTarget, 3782, timeSeconds);
                }
                if (mobTarget != null) {
        ...
        switch (player.playerSkill.skillSelect.template.id) {
                        ...
                        if (!player.isBoss) {
                            ...
                            for (Player pl : playersMap) {
                                ...
                                if (!pl.isBoss && pl.gender != ConstPlayer.NAMEC
                                    ...
                                    pl.nPoint.setHp(player.nPoint.hp + (int) ((long) player.nPoint.hp * tileHP / 100));
                                    Service.gI().point(pl);
                                    Service.gI().Send_Info_NV(pl);
                                }
                            }
            ...
            case Skill.TU_SAT:
                if (!player.playerSkill.prepareTuSat) {
                    ...
                    if (player.setClothes.cadicM >= 5) {
                        dame = (long) (player.nPoint.hpMax * 1.5);
                    } else if (player.setClothes.cadicM == 4) {
                        dame = (long) (player.nPoint.hpMax * 1.2);
                    }
                    if (!player.isBoss) {
                        for (Mob mob : player.zone.mobs) {
                    ...
                    if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                        for (Player pl : playersMap) {
                            if (!player.equals(pl) && canAttackPlayer(player, pl)
                                ...
                                pl.injured(player, dameAttack, MapService.gI().isMapYardart(player.zone.map.mapId),
                                        false);
    ...
    private void phanSatThuong(Player plAtt, Player plTarget, double dame) {
        if (plAtt != null && plAtt.nPoint != null && plTarget != null && plTarget.nPoint != null) {
            ...
            if (percentPST != 0) {
                double damePST = (int) (long) (dame * percentPST / 100L);
                Message msg = null;
                try {
                    msg = new Message(56);
                    ...
                    if (plAtt.isBoss
                            && !(plAtt instanceof boss.broly.Broly || plAtt instanceof boss.broly.SuperBroly)) {
                        if (damePST > plAtt.nPoint.hpMax / 100) {
                            long giamdame = 0;
                            if (plAtt.nPoint.hpMax / 200 > 1) {
                                giamdame = Util.maxIntValue(plAtt.nPoint.hpMax / 200);
                            }
                            damePST = plAtt.nPoint.hpMax / 100 - giamdame;
                        }
                    }
                    damePST = plAtt.injured(plAtt, damePST, true, false);
                    msg.writeSmartLong(Util.maxIntValue(plAtt.nPoint.hp));
                    msg.writeSmartLong(Util.maxIntValue(damePST));
    ...
    private void hutHPMP(Player player, double dame, boolean attackMob) {
        ...
        int hpHoi = (int) Util.maxIntValue(dame * tiLeHutHp / 100);
        int mpHoi = (int) Util.maxIntValue(dame * tiLeHutMp / 100);
        if (hpHoi > 0 || mpHoi > 0) {
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        if (plAtt == null || plAtt.nPoint == null || plAtt.playerSkill == null
                || plInjure == null || plInjure.effectSkill == null || plInjure.nPoint == null) {
            return;
        }
        if (plInjure.effectSkill.anTroi) {
            plAtt.nPoint.isCrit100 = true;
        }
        long dameAttack = miss ? 0 : plAtt.nPoint.getDameAttack(false);

        long satThuongChuan = plAtt.nPoint.getSatThuongChuan(dameAttack);
        double dameBiGiam = plInjure.injured(plAtt, dameAttack, false, false);
        double dameSTC = 0;
        if (satThuongChuan > 0 && !plInjure.isDie()) {
            dameSTC = plInjure.injured(plAtt, satThuongChuan, true, false);
        }
        double dameFinal = dameBiGiam + dameSTC;

        // Đảm bảo dameFinal không âm
        if (dameFinal < 0) {
            dameFinal = 0;
        }

        phanSatThuong(plAtt, plInjure, dameAttack);
        hutHPMP(plAtt, dameFinal, false);
        ...
        try {
            ...
            msg.writer().writeByte(typeSkill == 2 ? 0 : 1);
            msg.writer().writeByte(0);

            if (dameFinal > 2123456789) {
                if (plAtt.isPl()) {
                    plAtt.Hppl = "\n|8|Name Boss:\b|4|" + plInjure.name;
                    plAtt.Hppl += "\n|8|Hp Boss:\b|7|" + Util.powerToString(plInjure.nPoint.hp);
                    if (dameFinal > 1) {
                        plAtt.Hppl += "\b|8|Dame lên Boss:\b|7|" + Util.powerToString((long) dameFinal);
                    } else {
                        plAtt.Hppl += "\nDame lên Boss: HỤT";
                    }
                }
                if (Util.readInt && dameFinal > 2123455999) {
                    Service.gI().sendThongBao(plAtt, "|4|Dame thật \b|5|" + "" + Util.format(dameFinal));
                }
                msg.writeSmartLong(Util.maxIntValue(2123456000));
            } else {
                if (plAtt.isPl()) {
                    plAtt.Hppl = "\n|8|Name Boss:\b|4|" + plInjure.name;
                    plAtt.Hppl += "\n|8|Hp Boss:\b|7|" + Util.powerToString(plInjure.nPoint.hp);
                ...
                msg.writeSmartLong(Util.maxIntValue(dameFinal));
            }
            ...
            try {
                ...
                msg.writer().writeInt((int) plInjure.id);
                msg.writeSmartLong(Util.maxIntValue(plInjure.nPoint.hp));
                msg.writer().writeByte(0);
                msg.writeSmartLong(Util.maxIntValue(plInjure.nPoint.hpMax));
                Service.gI().sendMessAnotherNotMeInMap(plInjure, msg);
                if (plInjure.isBoss) {
                    PlayerService.gI().changeAndSendTypePK(plInjure, plInjure.typePk);
        ...
        if (Util.readInt && plInjure.isBoss && plInjure.isPl()) {
            try {
                long hp = plInjure.nPoint.hp;
                long hpMax = plInjure.nPoint.hpMax;
                if (hp > 2123455999 || hpMax > 2123455999) {
                    msg = new Message(44);
                    msg.writer().writeInt((int) plInjure.id);
                    msg.writer().writeUTF("|2|HP \b|7| " + Util.format(hp));
    ...
    private void playerAttackMob(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) {
        ...
        long dameAttack = plAtt.nPoint.getDameAttack(true);
        long satThuongChuan = plAtt.nPoint.getSatThuongChuan(dameAttack);
        long dameFinal = dameAttack + satThuongChuan;
        long dameHit = dameFinal;
        ...
        if (plAtt.charms != null && plAtt.charms.tdBatTu > System.currentTimeMillis() && plAtt.nPoint.hp <= 1) {
            ...
            if (plAtt.itemTime != null && plAtt.itemTime.timeHallowen > System.currentTimeMillis()
                    && plAtt.nPoint.hp <= 1) {
                if (plAtt.nPoint.hp < 1) {
                    plAtt.nPoint.hp = 1;
                }
    ...
    /** Hiệu ứng nổ tức thời dùng bởi option 153, không phụ thuộc kỹ năng đang chọn. */
    public void sendPlayerBomEffect(Player player, short skillId) {
        Message msg = null;
        try {
            msg = new Message(-45);
            msg.writer().writeByte(7);
    ...
    public boolean canUseSkillWithMana(Player player) {
        if (player.playerSkill.skillSelect != null) {
            ...
            switch (player.playerSkill.skillSelect.template.manaUseType) {
                case 0 -> {
                    return player.nPoint.mp >= getActualManaUse(player, player.playerSkill.skillSelect.manaUse);
                }
                case 1 -> {
    ...
    private void setMpAffterUseSkill(Player player) {
        if (player.playerSkill.skillSelect != null) {
            switch (player.playerSkill.skillSelect.template.manaUseType) {
                case 0 -> {
                    int mpUse = getActualManaUse(player, player.playerSkill.skillSelect.manaUse);
    ...
    /** ID 247 (4 món) còn 70%, ID 248 (5 món) còn 30% hao KI Kaioken. */
    private int getActualManaUse(Player player, int baseManaUse) {
        if (player.playerSkill != null && player.playerSkill.skillSelect != null
                && player.playerSkill.skillSelect.template.id == Skill.KAIOKEN) {
            if (player.setClothes.thanVuTruKaio == 4) {
                return baseManaUse * 70 / 100;
            }
            if (player.setClothes.thanVuTruKaio == 5) {
                return baseManaUse * 30 / 100;
            }
        }
        return baseManaUse;
    }
        ...
        switch (skillId) {
            ...
            case Skill.MASENKO -> {
                ...
                if (player.setClothes.nail == 4) {
                    subTimeParam = subTimeParam + 20;
                }

                if (player.setClothes.nail == 5) {
                    subTimeParam = subTimeParam + 50;
                }
            }
        }
