...
public class SnakeWayService {
		...
		if (clan != null) {
			...
			if (cm != null) {
				...
				if (player.clan.ConDuongRanDoc == null) {
					if (level == 0) {
						Service.gI().sendThongBao(player, "Hãy chọn cấp độ Con đường rắn độc");
						return;
					}
					if (level >= 1 && level <= 110) {
						...
						if (conDuongRanDoc != null) {
							if (Util.isAfterMidnight(player.clan.lastTimeOpenConDuongRanDoc)) {
								player.clan.timesPerDayCDRD = 1;
							} else {
								player.clan.timesPerDayCDRD++;
								if (player.clan.timesPerDayCDRD > 3) {
    ...
    public boolean finishNow(Player admin) {
        if (admin == null || admin.clan == null) {
            return false;
        }
        SnakeWay cdrd = admin.clan.ConDuongRanDoc;
        if (cdrd == null || !cdrd.isOpened()) {
            return false;
        }
        try {
            cdrd.finish();
