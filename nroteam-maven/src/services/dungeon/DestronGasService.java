...
public class DestronGasService {
    ...
    public boolean finishNow(Player admin) {
        ...
        DestronGas kghd = admin.clan.KhiGasHuyDiet;
        if (kghd == null || !kghd.isOpened()) {
            return false;
        }
        try {
            kghd.finish();
                        ...
                        if (clan.isLeader(player)) {
                            ...
                            if (khiGasHuyDiet != null) {
                                ...
                                if (Util.isAfterMidnight(player.clan.lastTimeOpenKhiGasHuyDiet)) {
                                    player.clan.timesPerDayKGHD = 0;
                                }
                                
                                // Kiểm tra giới hạn 3 lần/ngày
                                if (player.clan.timesPerDayKGHD >= 3) {
                                    Service.gI().sendThongBao(player, "Bạn đã vào khí gas hủy diệt 3 lần trong hôm nay, hẹn gặp lại ngày mai");
                                    return;
                                }
                                
                                khiGasHuyDiet.openKhiGasHuyDiet(player, player.clan, level);
                                
                                // Tăng counter sau khi mở thành công
