...
public class WorldMartialArtsTournamentService extends ConstTournament {
    ...
    public static void menu(Npc npc, Player player) {
        ...
        menu.add("Thông tin\nChi tiết");
        if (player.playerTask.taskMain.id == 16 && player.playerTask.taskMain.index == 1 && player.vip >= 1) {
            menu.add("Bỏ qua\n Nhiệm vụ");
        }
        if (canReg && tour != -1) {
            if (!regCheck(player)) {
    ...
    public static void confirm(Npc npc, Player player, int select) {
        ...
        switch (player.idMark.getIndexMenu()) {
            ...
            case ConstNpc.BASE_MENU -> {
                boolean hasSkipOption = player.playerTask.taskMain.id == 16
                        && player.playerTask.taskMain.index == 1 && player.vip >= 1;
                if (hasSkipOption && select == 1) {
                    if (TaskService.gI().doneTask(player, ConstTask.TASK_16_1)) {
                        Service.gI().sendThongBao(player, "Bạn đã hoàn thành nhiệm vụ");
                    }
                    return;
                }
                int s = select;
                if (hasSkipOption && s > 1) {
                    s--;
                }
                switch (s) {
                    case 0 ->
