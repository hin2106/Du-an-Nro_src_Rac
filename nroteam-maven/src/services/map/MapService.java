...
public class MapService {
    ...
    public Zone getMapCanJoin(Player player, int mapId, int zoneId) {
        ...
        if (mapId == 126 && (player.zone == null || !TimeUtil.is22H())) {
            Zone zone = getZone(21 + player.gender);
            player.location.x = Util.nextInt(100, zone.map.mapWidth - 100);
            player.location.y = zone.map.yPhysicInTop(player.location.x, 100);
            return zone;
        ...
        if (this.isMapCL(mapId) && (player.zone == null || player.clan == null || player.clan.bossChallenge == null)) {
        ...
        if (this.isMapCL(mapId) && player.zone != null && player.clan.bossChallenge != null) {
            if (this.isMapCL(player.zone.map.mapId)) {
                boolean allCharactersDead = true;
                for (Mob mob : player.clan.bossChallenge.getMapById(mapId).mobs) {
                    if (!mob.isDie()) {
                        allCharactersDead = false;
                        break;
                    }
                ...
                if (allCharactersDead) {
                    for (Player boss : player.clan.bossChallenge.getMapById(mapId).getBosses()) {
                        if (!boss.isDie()) {
                            allCharactersDead = false;
                            break;
                        }
                ...
                if (allCharactersDead) {
                    return player.clan.bossChallenge.getMapById(mapId);
                }
                for (Player boss : player.zone.getBosses()) {
                    if (!boss.isDie()) {
                        return null;
    ...
    public void changeToMap(Player player, int mapId, int zoneId) {
        try {
            Zone zoneJoin = getMapCanJoin(player, mapId, zoneId);
            if (zoneJoin == null) {
                Service.gI().sendThongBao(player, "Không thể vào bản đồ này!");
                return;
            }
    ...
    public Zone getZone(int mapId) {
        ...
        // All zones are full; return first zone so caller can handle capacity logic.
        ...
        try {
            ...
            if (map != null) {
                ...
                if (zoneId < 0 || zoneId >= map.zones.size()) {
                    Logger.warningln("[MapService] Invalid zoneId=" + zoneId
                            + " for mapId=" + mapId + " (zones=" + map.zones.size() + "), fallback auto-zone");
                    return getZone(mapId);
                }
    ...
    public Map getMapById1(int mapId) {
    ...
    public List<Zone> getMapCapsule(Player pl) {
        ...
        if (pl.mapBeforeCapsule != null
                ...
                && !isMapCL(pl.mapBeforeCapsule.map.mapId)
    ...
    public void sendPlayerMove(Player player) {
        ...
        if (player.isPl() || player.isBot || player.isBoss || player.isPet || player.isNewPet) {
            Message msg = null;
            try {
                msg = new Message(-7);
    ...
    public boolean isMapCL(int mapId) {
        return mapId == 164;
    }

    public boolean isMap3Lang(int mapId) {
        return mapId == 0 || mapId == 7 || mapId == 14;
    }
