...
public class ChangeMapService {
    ...
    public void openZoneUI(Player pl) {
        ...
        try {
            ...
            for (Zone zone : pl.zone.map.zones) {
                ...
                // Tab chọn khu phải hiển thị mọi nhân vật mà client nhìn thấy,
                // gồm player thường, admin và bot. Giới hạn vào khu vẫn dùng
                // getNumOfPlayers() để giữ luật admin không chiếm slot.
                int numPlayers = zone.getNumOfPlayersForDisplay();
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (!pl.isAdmin() && !pl.isBoss) {
            if (MapService.gI().isMapOffline(pl.zone.map.mapId)
                    ...
                    || MapService.gI().isMapMabu2H(pl.zone.map.mapId)) {
                NpcService.gI().createTutorial(pl, -1, "Không thể đến khu vực này");
                return;
            }
        }

        int defaultTimeChangeZone = 15000;
        int timeChangeZone = defaultTimeChangeZone;

        if (pl.traning()) {
            timeChangeZone = 6000;
        }

        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.idMark.getLastTimeChangeZone(), timeChangeZone)) {
            ...
            NpcService.gI().createTutorial(pl, -1, "Chưa thể chuyển khu vực lúc này vui lòng chờ "
                    + TimeUtil.getTimeLeft(pl.idMark.getLastTimeChangeZone(), timeChangeZone / 1000) + " nữa");
        }
    }
    ...
    /**
     * Chuyển map đến boss với mapId và zoneId cụ thể (chính xác) Teleport trực
     * tiếp đến zone được chỉ định mà không qua các check có thể thay đổi zone
     *
     * @param pl
     * @param mapId
     * @param zoneId
     ...
     * @param y
     */
    public void changeMapBoss(Player pl, int mapId, int zoneId, int x, int y) {
        Zone zoneJoin = MapService.gI().getZoneByMapIDAndZoneID(mapId, zoneId);
        if (zoneJoin == null) {
            Service.gI().sendThongBao(pl, "Không tìm thấy map hoặc zone!");
            return;
        }
        changeMapBossDirect(pl, zoneJoin, x, y);
    }

    /**
     * Chuyển map đến boss với zone đã xác định (teleport trực tiếp)
     *
     * @param pl
     * @param zoneJoin
     * @param x
    ...
    private void changeMapBossDirect(Player pl, Zone zoneJoin, int x, int y) {
        if (zoneJoin != null) {
            ...
            pl.location.x = resolveMapX(zoneJoin, x);
            pl.location.y = resolveMapY(zoneJoin, pl.location.x, y);
            this.goToMap(pl, zoneJoin);
            boolean isOfflineMap = MapService.gI().isMapOffline(zoneJoin.map.mapId);
            if (isOfflineMap) {
                if (pl.pet != null) {
                    if (pl.pet.zone != null) {
                        this.exitMap(pl.pet);
                    }
                    Zone homeZone = MapService.gI().getMapCanJoin(pl.pet, pl.gender + 21, -1);
                    if (homeZone != null) {
                        ChangeMapService.gI().goToMap(pl.pet, homeZone);
                        if (pl.pet.zone != null) {
                            pl.pet.zone.load_Me_To_Another(pl.pet);
                        }
                    }
                }
                if (pl.newPet != null) {
                    if (pl.newPet.zone != null) {
                        this.exitMap(pl.newPet);
                    }
                    Zone homeZone = MapService.gI().getMapCanJoin(pl.newPet, pl.gender + 21, -1);
                    if (homeZone != null) {
                        ChangeMapService.gI().goToMap(pl.newPet, homeZone);
                        if (pl.newPet.zone != null) {
                            pl.newPet.zone.load_Me_To_Another(pl.newPet);
                        }
                    }
                }
            } else {
            ...
            if (!pl.isPl()) {
                ...
                zoneJoin.load_Another_To_Me(pl);
    ...
    private int resolveMapX(Zone zoneJoin, int x) {
        if (zoneJoin == null || zoneJoin.map == null) {
            return x;
        }
        int minX = 60;
        int maxX = Math.max(minX, zoneJoin.map.mapWidth - 60);
        if (x == -1) {
            if (zoneJoin.map.mapWidth - 100 > 100) {
                return Util.nextInt(100, zoneJoin.map.mapWidth - 100);
            }
            return 100;
        }
    ...
    private int resolveMapY(Zone zoneJoin, int x, int y) {
        if (zoneJoin == null || zoneJoin.map == null) {
            return y;
        }
        int mapHeight = Math.max(24, zoneJoin.map.mapHeight);
        int groundY = zoneJoin.map.yPhysicInTop(x, 100);
        if (groundY <= 0 || groundY > mapHeight) {
            groundY = Math.min(mapHeight - 24, Math.max(24, y));
        }
        if (y < 0 || y > mapHeight || Math.abs(y - groundY) > 2000) {
            return groundY;
        }
        return Math.max(0, y);
    }
    ...
    public void finishLoadMap(Player player) {
        ...
        try {
            // Player được tải từ database đã có sẵn zone nên không đi qua goToMap().
            // Đăng ký lại ở đây (addPlayer có chống trùng) để danh sách khu và giới
            // hạn /maxPlayer đếm đủ cả player thật lẫn bot.
            player.zone.addPlayer(player);
            TaskService.gI().spawnBabyItemForPlayer(player);
            player.zone.load_Me_To_Another(player);
            player.zone.load_Another_To_Me(player);

            // QUAN TRỌNG: Gửi effect set cho chính bản thân sau khi load xong
            if (player.setClothes != null && player.setClothes.hasSetEffect()) {
                player.setClothes.sendSetEffectToPlayer(player);
            }
        } catch (Exception e) {
            Logger.logException(ChangeMapService.class, e, "Error loading players");
        }

        MapLoadOptimizer.gI().delayNonCriticalUpdates(player, () -> {
            try {
                sendEffectMapToMe(player);
                sendEffectMeToMap(player);
            } catch (Exception e) {
                Logger.logException(ChangeMapService.class, e, "Error sending effects");
            }
        });
    ...
    private void sendEffectMeToMap(Player player) {
        ...
        try {
            ...
            if (player.mobMe != null) {
                ...
                msg.writeSmartLong(player.mobMe.point.gethp());
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
            if (player.pet != null && player.pet.mobMe != null) {
                ...
                msg.writeSmartLong(player.pet.mobMe.point.gethp());
                Service.gI().sendMessAnotherNotMeInMap(player, msg);
                msg.cleanup();
            }
        } catch (IOException e) {
    ...
    private void sendEffectMapToMe(Player player) {
        ...
        try {
            List<Player> players = new ArrayList<>(player.zone.getHumanoids());
            for (Player pl : players) {
                if (pl != null && !player.equals(pl)) {

                    if (pl.effectSkill != null && pl.effectSkill.isShielding) {
                        msg = new Message(-124);
                        msg.writer().writeByte(1);
                        msg.writer().writeByte(0);
                        msg.writer().writeByte(33);
                    ...
                    if (pl.effectSkill != null && pl.effectSkill.isThoiMien) {
                        msg = new Message(-124);
                        msg.writer().writeByte(1); // b5
                    ...
                    if (pl.effectSkill != null && (pl.effectSkill.isBlindDCTT || pl.effectSkill.isStun)) {
                        msg = new Message(-124);
                        msg.writer().writeByte(1);
                        msg.writer().writeByte(0);
                        msg.writer().writeByte(40);
                    ...
                    if (pl.mobMe != null) {
                        ...
                        msg.writeSmartLong(pl.mobMe.point.gethp());
                        player.sendMessage(msg);
                        msg.cleanup();
                    }
                }
    ...
    public void goToMap(Player player, Zone zoneJoin) {
        ...
        if (player.location != null) {
            player.zone.addPlayer(player);
            if (zoneJoin != null && zoneJoin.spatialGridEnabled && zoneJoin.spatialGrid != null
                    && !player.isBoss && !player.isPet && !player.isNewPet
                    && !(player instanceof npc.NonInteractiveNPC)) {
                try {
                    zoneJoin.spatialGrid.addPlayer(player);
                } catch (Exception e) {
                }
            }
        } else {
        ...
        if (zoneJoin != null && zoneJoin.map != null) {
            services.map.MapUpdateOptimizer.gI().markMapActive(zoneJoin.map.mapId);
            dragonpass.DragonPassService.gI().onMapVisited(player, zoneJoin.map.mapId);
        }
    }
    ...
    public void goToDBKB(Player player) {
        if (!player.idMark.isGoToBDKB()) {
            // Kiểm tra và reset counter nếu qua ngày mới
            if (Util.isAfterMidnight(player.lastTimeJoinBDKB)) {
                player.timesPerDayBDKB = 0;
            }
            
            // Chỉ tăng counter nếu chưa được tăng trong TreasureUnderSeaService
            // (tức là lastTimeJoinBDKB chưa được cập nhật với lastTimeOpenBanDoKhoBau)
            if (player.clan != null && player.clan.lastTimeOpenBanDoKhoBau > 0 
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {
                ...
                case 27, 28, 31, 32, 35, 36 -> {
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_11_0) {
                        return null;
                    }
                }
                case 30, 34, 38 -> {
                ...
                case 102, 92, 93, 94, 96, 97, 98, 99, 100 -> {
                    if (TaskService.gI().getIdTask(player) < ConstTask.TASK_21_0) {
                        return null;
                    }
                }
