...
public class TopBDKBService {

    private static TopBDKBService instance;

    private TopBDKBService() {
    }

    public static TopBDKBService gI() {
        if (instance == null) {
            instance = new TopBDKBService();
        }
        return instance;
    }
    ...
    public List<TopRowService> queryTop(int limit) {
        ...
        try {
            String sql = "SELECT id, name, thanhtichbdkb, thongtin FROM clan WHERE thanhtichbdkb IS NOT NULL";
            AlyraResultSet rs = AlyraManager.executeQuery(sql);
            while (rs.next()) {
                String tt = rs.getString("thanhtichbdkb");
            ...
            Logger.error("TopBDKBService.queryTop error: " + e);
        }
    ...
    public void showTop(Player viewer) {
        ...
        try {
            ...
            Logger.error("TopBDKBService.showTop error: " + e);
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
