...
public class TopKhiGaService {

    private static TopKhiGaService instance;

    public static TopKhiGaService gI() {
        if (instance == null) {
            instance = new TopKhiGaService();
        }
        return instance;
    }
    ...
    public List<TopRowService> queryTop(int limit) {
        ...
        try {
            String sql = "SELECT id, name, thanhtichkghd, thongtin FROM clan WHERE thanhtichkghd IS NOT NULL";
            AlyraResultSet rs = AlyraManager.executeQuery(sql);
            while (rs.next()) {
                String tt = rs.getString("thanhtichkghd");
            ...
            Logger.error("TopKhiGaService.queryTop error: " + e);
        }
        list.sort(Comparator
                .comparingInt((TopRowService r) -> r.level != null ? r.level : 0).reversed()
                .thenComparingLong(r -> r.time != null ? r.time : 0L));
        if (list.size() > limit) {
    ...
    public void showTop(Player viewer) {
        ...
        try {
            ...
            msg.writer().writeUTF("Top 100 Khí Ga");
            ...
            Logger.error("TopKhiGaService.showTop error: " + e);
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
