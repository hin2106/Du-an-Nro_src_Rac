...
public class EffectSkillService {
    ...
    public void setThoiMien(Player player, long lastTimeThoiMien, int timeThoiMien) {
        ...
        if (player.effectSkill.isThoiMien) {
            ...
            long currentElapsed = currentTime - player.effectSkill.lastTimeThoiMien;
            if (currentElapsed >= player.effectSkill.timeThoiMien) {
            } else {
                long remainingTime = player.effectSkill.timeThoiMien - currentElapsed;

                if (remainingTime > 500) {
                    System.out.println("[setThoiMien] Player already has ThoiMien, remaining: " + remainingTime
                            + "ms, ignoring new apply");
                    return;
                }
                if (timeThoiMien <= remainingTime + 1000) {
                    return;
                }
            }
        }
    ...
    public void removeThoiMien(Player player) {
        ...
        ItemTimeService.gI().removeItemTime(player, 3782);
    ...
    public void setIsMonkey(Player player) {
        ...
        int defaultTimeMonkey = SkillUtil.getTimeMonkey(player.playerSkill.skillSelect.point);

        boolean hasCadicSet = player.setClothes != null && player.setClothes.checkSetCadic();
        int timeMonkey = defaultTimeMonkey;
        if (hasCadicSet) {
            timeMonkey = defaultTimeMonkey * 5;
        }

        // Lưu thông tin vào effectSkill
        // levelMonkey MUST be set before isMonkey=true to avoid race condition:
        // if another thread calls getHead() after isMonkey=true but before levelMonkey is set,
        // HEADMONKEY[levelMonkey-1] = HEADMONKEY[-1] → ArrayIndexOutOfBoundsException → head disappears
        ...
        player.effectSkill.defaultTimeMonkey = defaultTimeMonkey;
        player.effectSkill.hadCadicSetWhenStart = hasCadicSet;
    ...
    public void removeChibi(Player player) {
        ...
        player.effectSkill.timeChibi = 0;
    ...
    public void setIsHalloween(Player player, int outFit, int time) {
        if (player.effectSkill != null) {
            ...
            if (player.itemTime != null) {
                player.itemTime.isHallowen = true;
                player.itemTime.timeHallowen = time;
                player.itemTime.lastTimeHallowen = System.currentTimeMillis();
                player.itemTime.idOutfitHalloween = player.effectSkill.idOutfitHalloween;
            }
    ...
    public void removeHalloween(Player player) {
        if (player.effectSkill != null) {
            ...
            ItemTimeService.gI().sendItemTime(player, 5101, 0);
            Service.gI().Send_Caitrang(player);
        }
    }

    public void setCarrot(Player player, int time) {
        player.effectSkill.isCarrot = true;
        player.effectSkill.timeCarrot = time;
        player.effectSkill.lastTimeCarrot = System.currentTimeMillis();
        if (player.itemTime != null) {
            player.itemTime.isUseCarrot = true;
            player.itemTime.timeCarrot = time;
            player.itemTime.lastTimeCarrot = System.currentTimeMillis();
        }
        ItemTimeService.gI().sendItemTime(player, 4083, time / 1000);
        Service.gI().Send_Caitrang(player);
        Service.gI().point(player);
        Service.gI().Send_Info_NV(player);
    }
    ...
    public void removeCarrot(Player player) {
        ...
        ItemTimeService.gI().sendItemTime(player, 4083, 0);
        Service.gI().Send_Caitrang(player);
        Service.gI().point(player);
        if (player.itemTime != null) {
