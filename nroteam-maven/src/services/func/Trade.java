...
public class Trade {
    ...
    public static final long QUANLITY_MAX = 2_000_000_000;
    public static final int MAX_GOLD_TRADE_PER_TIME = 2_000_000_000;
    ...
    public void addItemTrade(Player pl, byte index, int quantity) {
        if (index == -1) { 
            ...
            if (quantity > 999) {
                ...
                Service.gI().sendThongBao(pl, "Số lượng một vật phẩm giao dịch không được vượt quá 999.");
                return;
            }
            ...
            if (isItemCannotTran(item)) {
                ...
                item.ensureAuditTraceId();
                int currentTotalQuantity = 0;
                List<Item> currentTradeList = pl.equals(this.player1) ? itemsTrade1 : itemsTrade2;
                
                for (Item it : currentTradeList) {
                    currentTotalQuantity += it.quantity;
                }

                if (currentTotalQuantity + quantity > 2000) {
                    removeItemTrade(pl, index); 
                    Service.gI().sendThongBao(pl, "Tổng số lượng tất cả vật phẩm giao dịch không được vượt quá 2000.");
                    return;
                }
        ...
        if (tradeStatus != SUCCESS) {
            ...
            if (tradeStatus != SUCCESS) {
                ...
                if (tradeStatus == SUCCESS) {
                    ...
                    for (Item item : itemsTrade1) {
                        AssetAuditService.gI().recordItem("TRADE_ITEM", item.auditTraceId, item, item.quantity,
                                player1, player2, player1, "Giao dịch trực tiếp giữa hai người chơi");
                    }
                    for (Item item : itemsTrade2) {
                        AssetAuditService.gI().recordItem("TRADE_ITEM", item.auditTraceId, item, item.quantity,
                                player2, player1, player1, "Giao dịch trực tiếp giữa hai người chơi");
                    }
                    if (goldTrade1 > 0) AssetAuditService.gI().recordCurrency("TRADE_GOLD", "GOLD", goldTrade1,
                            player1, player2, player1, "Giao dịch trực tiếp giữa hai người chơi");
                    if (goldTrade2 > 0) AssetAuditService.gI().recordCurrency("TRADE_GOLD", "GOLD", goldTrade2,
                            player2, player1, player1, "Giao dịch trực tiếp giữa hai người chơi");
                    AssetAuditService.gI().markExactBalanceHandled();
                }
                sendNotifyTrade(tradeStatus);
                this.closeTab();
                this.dispose(); 
