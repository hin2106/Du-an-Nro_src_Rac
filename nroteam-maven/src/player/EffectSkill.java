...
public class EffectSkill {
    ...
    public int defaultTimeMonkey;
    public boolean hadCadicSetWhenStart;

    // biên broly
    public boolean isBienBroly;
    public byte levelBienBroly = 0;
    public long lastTimeUpBienBroly;
    public int timeBienBroly;
    ...
    public void update() {
        if (isMonkey) {
            boolean hasCadicSetNow = player.setClothes != null && player.setClothes.checkSetCadic();
            long currentTime = System.currentTimeMillis();
            long elapsedTime = currentTime - lastTimeUpMonkey;
            if (hadCadicSetWhenStart && !hasCadicSetNow) {
                if (elapsedTime >= defaultTimeMonkey) {
                    EffectSkillService.gI().monkeyDown(player);
                    return;
                } else {
                    timeMonkey = defaultTimeMonkey;
                    hadCadicSetWhenStart = false; 
                }
            }
            if (Util.isTimeExpired(lastTimeUpMonkey, timeMonkey)) {
                EffectSkillService.gI().monkeyDown(player);
            }
        }
        if (isBinh && Util.isTimeExpired(lastTimeUpBinh, timeBinh)) {
            EffectSkillService.gI().BinhDown(player);
        }
        if (isShielding && Util.isTimeExpired(lastTimeShieldUp, timeShield)) {
            EffectSkillService.gI().removeShield(player);
        }
        if (useTroi && (Util.isTimeExpired(lastTimeTroi, timeTroi)
                || plAnTroi != null && plAnTroi.isDie()
                || useTroi && isHaveEffectSkill())) {
            EffectSkillService.gI().removeUseTroi(this.player);
        }
        if (isStun && (Util.isTimeExpired(lastTimeStartStun, timeStun) || player.isDie())) {
            EffectSkillService.gI().removeStun(this.player);
        }
        if (isThoiMien && (Util.isTimeExpired(lastTimeThoiMien, timeThoiMien) || player.isDie())) {
            EffectSkillService.gI().removeThoiMien(this.player);
        }
        if (isBlindDCTT && (Util.isTimeExpired(lastTimeBlindDCTT, timeBlindDCTT) || player.isDie())) {
            EffectSkillService.gI().removeBlindDCTT(this.player);
        }
        if (isSocola && Util.isTimeExpired(lastTimeSocola, timeSocola)) {
