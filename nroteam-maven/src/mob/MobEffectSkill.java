...
public class MobEffectSkill {
    ...
    public void update() {
        if (isStun && (Util.isTimeExpired(lastTimeStun, timeStun) || mob.isDie())) {
            removeStun();
        }
        if (isThoiMien && (Util.isTimeExpired(lastTimeThoiMien, timeThoiMien) || mob.isDie())) {
            removeThoiMien();
        }
        if (isBlindDCTT && (Util.isTimeExpired(lastTimeBlindDCTT, timeBlindDCTT) || mob.isDie())) {
            removeBlindDCTT();
        }
        if (isSocola && (Util.isTimeExpired(lastTimeSocola, timeSocola) || mob.isDie())) {
            removeSocola();
        }
        if (isAnTroi && (Util.isTimeExpired(lastTimeAnTroi, timeAnTroi) || mob.isDie())) {
            removeAnTroi();
        }
        if (this.isBinh) {
            ...
            if (Util.canDoWithTime(lastTimeMaPhongBa, 500) && !mob.isDie()) {
                if (playerUseMafuba != null && playerUseMafuba.playerSkill != null) {
                    long param = playerUseMafuba.playerSkill.getSkillbyId(Skill.MA_PHONG_BA).point;
                    long subHp = (int) ((long) playerUseMafuba.nPoint.hpMax * param
