...
public class GhiDanh extends Npc {
    ...
    public void openBaseMenu(Player pl) {
        if (canOpenNpc(pl)) {
            switch (this.mapId) {
                ...
                // case 42, 43, 44 -> {
                //     this.createOtherMenu(pl, ConstNpc.BASE_MENU,
                //             "Tính điểm máy đấm nào các thí sính\n Máy đấm đằng kia không phải tôi.",
                //             "Top 100\n Trái đất", "Top 100\n Namếc", "Top 100\n Xayda", "Xem điểm", "Đóng");
                // }
                default ->
    ...
    public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
            switch (this.mapId) {
                case 52 -> WorldMartialArtsTournamentService.confirm(this, player, select);
                case 129 -> {
                ...
                case 42, 43, 44 -> {
                    if (player.idMark.isBaseMenu()) {
                        switch (select) {
                            case 0 -> TopGhiDanhService.gI().showTopByGender(player, consts.ConstPlayer.TRAI_DAT);
                            case 1 -> TopGhiDanhService.gI().showTopByGender(player, consts.ConstPlayer.NAMEC);
                            case 2 -> TopGhiDanhService.gI().showTopByGender(player, consts.ConstPlayer.XAYDA);
                            case 3 -> TopGhiDanhService.gI().showSelfRank(player);
                            default -> {
                            }
                        }
                    }
