...
public class Cui extends Npc {
    ...
    private static final int COST_FIND_BOSS = 50_000_000;
            ...
            case 19 -> {
                if (player.idMark.isBaseMenu()) {
                    switch (select) {
                        ...
                        // case 2 -> ChangeMapService.gI().changeMapBySpaceShip(player, 190, -1, 1488);
                    }
                } else {
                    switch (indexMenu) {
                        case ConstNpc.MENU_FIND_KUKU -> {
                            if (select == 0) tryMoveToBoss(player, BossID.KUKU);
                            else handleMapChoice(player, select);
                        }
                        case ConstNpc.MENU_FIND_MAP_DAU_DINH -> {
                            if (select == 0) tryMoveToBoss(player, BossID.MAP_DAU_DINH);
                            else handleMapChoice(player, select);
                        }
                        case ConstNpc.MENU_FIND_RAMBO -> {
                            if (select == 0) tryMoveToBoss(player, BossID.RAMBO);
                            else handleMapChoice(player, select);
                        }
                    }
                }
    ...
    private void tryMoveToBoss(Player player, int bossId) {
    ...
    private void handleMapChoice(Player player, int select) {
        switch (select) {
