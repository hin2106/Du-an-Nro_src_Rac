...
public class LuckyNumber implements Runnable {
    ...
    static {
        LuckNumberData.timeGame = LuckNumberData.timeGameDefalue;
        LuckNumberData.timeDelay = 0;
        spinGame = true;
    }
    ...
    public void run() {
        while (!Maintenance.isRunning()) {
            try {
                if (isOpen()) {
                    if (LuckNumberData.timeGame > 0) { // xử lý đếm ngươc thời gian về 0
                        LuckNumberData.timeGame--;
                        rewardPlayerWin();
                    }
                    if (LuckNumberData.timeGame == 0 && spinGame) {
                        spinGame();
                    }
                    if (LuckNumberData.timeDelay > 0) {
                        LuckNumberData.timeDelay--;
                        if (LuckNumberData.timeDelay == 0) {
                            rewardWinGame();
                        }
                    }
    ...
    public static void checkPlayerWin(long id) {
        ...
        for (LuckNumberData g : players) {
            ...
            if (id == g.id && g.number == RESULT && !g.isReward && player != null) {
                if (g.isGem) {
                    finish = "Chúc mừng " + player.name + " đã thắng " + ConstMiniGame.COST_GEM + " ngọc với con số may mắn " + RESULT;
                    LuckyNumberService.addDataResul(player, g.number, ConstMiniGame.COST_GEM, finish);
                    g.isReward = true;
                } else {
                    finish = "Chúc mừng " + player.name + " đã thắng " + Util.number(ConstMiniGame.COST_GOLD) + " vàng với con số may mắn " + RESULT;
                    LuckyNumberService.addDataResul(player, g.number, ConstMiniGame.COST_GOLD, finish);
                    g.isReward = true;
                }
