...
public class LuckyNumberService {
    ...
    public static void addData(Player player, int number, boolean isGem) {
        ...
        data.isGem = isGem;
        data.isReward = false;
        LuckyNumber.players.add(data);
        ItemTimeService.gI().sendItemTime(player, 2295, LuckNumberData.timeGame);
    }
    ...
    public static List listNumberSelect(Player pl) {
        ...
        for (LuckNumberData data : LuckyNumber.players) {
            if (data.id == pl.id) {
    ...
    public static String strNumber(long id) {
        String number = "";
        List<LuckNumberData> pl = LuckyNumber.players.stream().filter(d -> d.id == id).toList();
    ...
    public static void addNumber(Player pl, int number) {
        ...
        if (LuckyNumber.players.stream().filter(d -> d.id == pl.id).toList().size() >= 10) {
            Service.gI().sendThongBao(pl, "Bạn đã chọn 10 số rồi không thể chọn thêm");
            return;
        }
        if (pl.iDMark.isGemCSMM()) {
            if (pl.inventory.gem < ConstMiniGame.COST_PLAY_GEM) {
                Service.gI().sendThongBao(pl, "Bạn không đủ ngọc, còn thiếu " + (ConstMiniGame.COST_PLAY_GEM - pl.inventory.gem) + " ngọc nữa");
                return;
            }
            pl.inventory.gem -= ConstMiniGame.COST_PLAY_GEM;
        } else {
            if (pl.inventory.gold < ConstMiniGame.COST_PLAY_GOLD) {
                Service.gI().sendThongBao(pl, "Bạn không đủ vàng, còn thiếu " + Util.numberToMoney(ConstMiniGame.COST_PLAY_GOLD - pl.inventory.gold) + " vàng nữa");
                return;
            }
            pl.inventory.gold -= ConstMiniGame.COST_PLAY_GOLD;
        }
   ...
   @SuppressWarnings("unchecked")
    public static void addOneNumber(Player pl, boolean isOdd) {
        ...
        // Tránh recursion không giới hạn
        if (list.contains(numberRandom)) {
            addOneNumber(pl, isOdd);
            return;
        }

        // ✅ Giả sử LuckyNumber.players là List<Player>
        long count = LuckyNumber.players.stream()
                .filter(d -> d.id == pl.id)
                .count();
        ...
        if (pl.iDMark.isGemCSMM()) {
            if (pl.inventory.gem < ConstMiniGame.COST_PLAY_GEM) {
                Service.gI().sendThongBao(pl, "Bạn không đủ ngọc, còn thiếu "
                        + (ConstMiniGame.COST_PLAY_GEM - pl.inventory.gem) + " ngọc nữa");
                return;
            }
            pl.inventory.gem -= ConstMiniGame.COST_PLAY_GEM;
        } else {
            if (pl.inventory.gold < ConstMiniGame.COST_PLAY_GOLD) {
                Service.gI().sendThongBao(pl, "Bạn không đủ vàng, còn thiếu "
                        + Util.numberToMoney(ConstMiniGame.COST_PLAY_GOLD - pl.inventory.gold) + " vàng nữa");
                return;
            }
            pl.inventory.gold -= ConstMiniGame.COST_PLAY_GOLD;
        }
