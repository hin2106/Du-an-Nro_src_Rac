...
public class Map {
    ...
    public Map(int mapId, String mapName, byte planetId,
        ...
        this.planetName = mapId == 84 || mapId == 154 ? "-" : Service.gI().get_HanhTinh(planetId);
        this.tileId = tileId;
        this.bgId = bgId;
        this.bgType = bgType;
        this.type = type;
    ...
    private void initZone(int nZone, int maxPlayer) {
        switch (this.type) {
            ...
            case ConstMap.MAP_MABU_14H ->
                nZone = MajinBuu14H.MIN_INSTANCES;
            ...
            case ConstMap.CHALLENGE_MAP ->
                nZone = BossChallenge.AVAILABLE;
        ...
        for (int i = 0; i < nZone; i++) {
            ...
            switch (this.type) {
                ...
                case ConstMap.CHALLENGE_MAP ->
                    BossChallengeService.gI().addMapBossChallenge(i, zone);
                case ConstMap.MAP_KHI_GAS_HUY_DIET ->
        ...
        for (int i = 0; i < mobTemp.length; i++) {
            ...
            if (temp != null) {
                ...
                for (Zone zone : this.zones) {
                    ...
                    // Thêm vào spatial grid khi mob spawn
                    if (zone.spatialGridEnabled && zone.spatialGrid != null
                            && mobZone.location != null && !mobZone.isDie()) {
                        try {
                            zone.spatialGrid.addMob(mobZone);
                        } catch (Exception e) {
                            utils.Logger.logException(Map.class, e, "Lỗi thêm mob vào spatial grid");
                        }
                    }
                }
            }
    ...
    public void initMob(List<Mob> mobs) {
        for (Zone zone : zones) {
            for (Mob m : mobs) {
                ...
                zone.mobs.add(mob);
                if (zone.spatialGridEnabled && zone.spatialGrid != null
                        && mob.location != null && !mob.isDie()) {
                    try {
                        zone.spatialGrid.addMob(mob);
                    } catch (Exception e) {
                        utils.Logger.logException(Map.class, e, "Lỗi thêm mob vào spatial grid");
                    }
                }
    ...
    public void removeNpcByTempId(int tempId) {
        if (this.npcs == null || this.npcs.isEmpty()) {
            return;
        }
        this.npcs.removeIf(n -> n != null && n.tempId == tempId);
    }

    public boolean hasNpcByTempId(int tempId) {
        if (this.npcs == null || this.npcs.isEmpty()) {
            return false;
        }
        for (Npc n : this.npcs) {
            if (n != null && n.tempId == tempId) {
                return true;
            }
        }
        return false;
