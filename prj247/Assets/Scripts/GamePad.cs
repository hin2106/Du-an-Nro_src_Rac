	...
	public void update()
	{
		try
		{
			...
			if (GameCanvas.isPointerDown && !GameCanvas.isPointerJustRelease)
			{
				...
				{
					int defaultX = 45;
					int defaultY = isLargeGamePad ? GameCanvas.h - 45 : GameCanvas.h - 90;
					int startRadius = R + 12;
					int startDx = xTemp - defaultX;
					int startDy = yTemp - defaultY;
					if (startDx * startDx + startDy * startDy > startRadius * startRadius)
					{
						return;
					}
					xC = (xM = defaultX);
