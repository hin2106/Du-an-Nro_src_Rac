public class PopUpYesNo : IActionListener
{
	...
	{
		ModFunc.HudPaintState requestState = ModFunc.BeginHudElement(g, ModFunc.HUD_REQUEST, "Yêu cầu thách đấu / giao dịch", X - 35, Y, W + 35, H + 20);
		...
		if (info != null)
		{
			...
			// TextInfo.paint đặt translate về 0 sau khi cắt dòng chữ. Áp lại phần
			// dịch chuyển của HUD để nút chấp nhận đi cùng khung thông báo.
			if (requestState.translateX != 0 || requestState.translateY != 0)
			{
				g.translate(requestState.translateX, requestState.translateY);
			}
		...
		ModFunc.EndHudElement(requestState);
	}
