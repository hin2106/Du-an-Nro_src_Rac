...
public class TField : IActionListener
{
	...
	{
		// Vùng xóa phải nằm hoàn toàn bên trong ô nhập. Vùng cũ rộng 40px và
		// tràn 20px ra ngoài nên đè lên nút Ẩn/Hiện đặt bên phải mật khẩu.
		const int clearHitWidth = 24;
		if (GameCanvas.isPointerHoldIn(x + width - clearHitWidth, y, clearHitWidth, height))
		{
			clearAllText();
			isFocus = true;
		}
		else if (GameCanvas.isPointerHoldIn(x, y, width - clearHitWidth, height))
		{
			setFocusWithKb(isFocus: true);
		}
		else
