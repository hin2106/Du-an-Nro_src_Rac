public class TabClanIcon : IActionListener
{
    ...
    public void updateKey()
    {
        ...
        if (!isGetName)
        {
            ...
            if (GameCanvas.isTouch)
            {
                ...
                if (GameCanvas.isPointer(x, y, w, h))
                {
                    scrMain.updateKey();
                    select = scrMain.selectedItem;

                    if (select >= 0 && select < nItem)
                    {
                        lastSelect = select;
                    }
                    else
            ...
            if (select != -1 && select >= 0 && select < nItem)
            {
                lastSelect = select;
            }
        }
    ...
    public void perform(int idAction, object p)
    {
        ...
        if (!isRequest)
        {
            ...
            if (lastSelect >= 0 && lastSelect < ClanImage.vClanImage.size())
            {
                hide();
                if (Char.myCharz().clan == null)
                {
        ...
        else if (lastSelect >= 0 && lastSelect < vItems.size())
        {
