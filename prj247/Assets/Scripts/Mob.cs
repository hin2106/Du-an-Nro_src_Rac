...
public class Mob : IMapObject
{
    ...
    public void updateHp_bar()
    {
            ...
            int currentPercent = (int)((double)((long)hp * 100) / maxHp);
            if (currentPercent < 0)
            {
                currentPercent = 0;
            }
    ...
    protected void paintRoundedHpBar(mGraphics g, int barX, int barY, int barWidth, int barHeight, int currentWidth, int delayedWidth)
    {
        if (barWidth <= 0 || barHeight <= 0)
        {
            return;
        }
        currentWidth = System.Math.Max(0, System.Math.Min(barWidth, currentWidth));
        delayedWidth = System.Math.Max(currentWidth, System.Math.Min(barWidth, delayedWidth));
        int radius = System.Math.Max(1, barHeight / 2);
        g.setColor(4473924);
        g.FillRoundRect(barX - 1, barY - 1, barWidth + 2, barHeight + 2, 0, radius + 1);
        g.setColor(6447714);
        g.FillRoundRect(barX, barY, barWidth, barHeight, 0, radius);
        if (delayedWidth > 0)
        {
            g.setColor(16777215);
            g.FillRoundRect(barX, barY, delayedWidth, barHeight, 0, radius);
        }
        if (currentWidth > 0)
        {
            g.setColor(color);
            g.FillRoundRect(barX, barY, currentWidth, barHeight, 0, radius);
        }
    }
    ...
    public virtual void paint(mGraphics g)
    {
        ...
        int healthBarX = x - (imageWidth >> 1);
        int healthBarY = y - h - 5;
        paintRoundedHpBar(g, healthBarX, healthBarY, imageWidth, imageHeight, num, num2);
    }
