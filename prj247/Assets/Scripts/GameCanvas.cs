...
public class GameCanvas : IActionListener
{
    ...
    public void update()
    {
        ...
        try
        {
            ...
            {
                bool dragonPassCapturedPointer = false;
                bool dragonPassPointerClick = false;
                bool dragonPassPointerDown = false;
                bool dragonPassPointerJustDown = false;
                bool dragonPassPointerJustRelease = false;
                if (DragonPass.DragonPassUI.Visible)
                {
                    DragonPass.DragonPassUI.UpdateInput();
                    dragonPassCapturedPointer = true;
                    dragonPassPointerClick = isPointerClick;
                    dragonPassPointerDown = isPointerDown;
                    dragonPassPointerJustDown = isPointerJustDown;
                    dragonPassPointerJustRelease = isPointerJustRelease;
                ...
                if (dragonPassCapturedPointer && DragonPass.DragonPassUI.Visible)
                {
                    isPointerClick = dragonPassPointerClick;
                    isPointerDown = dragonPassPointerDown;
                    isPointerJustDown = dragonPassPointerJustDown;
                    isPointerJustRelease = dragonPassPointerJustRelease;
                }
                Hint.update();
                SoundMn.gI().update();
            }
    ...
    public void keyPressedz(int keyCode)
    {
        ...
        if (DragonPass.DragonPassUI.HandleKeyPress(keyCode))
        {
            keyAsciiPress = 0;
            return;
        }
    ...
    public void paint(mGraphics gx)
    {
        try
        {
            ...
            {
                ModFunc.PaintHudEditorOverlay(g);
            }
    ...
    public static Image loadImageRMS(string path)
    {
        ...
        Image hdImage = loadImageResize(pathOrigin);
        if (hdImage != null)
        {
            return hdImage;
        }
        path = cutPng(Main.res + "/x" + mGraphics.assetZoomLevel + pathOrigin);
        Image result = null;
        try
        {
        ...
        catch (Exception ex)
        {
            try
            {
                ...
                sbyte[] array2 = Rms.loadRMS("x" + mGraphics.assetZoomLevel + array[array.Length - 1]);
                if (array2 != null)
                {
                    result = Image.createImage(array2, 0, array2.Length);
                    prepareAssetImage(result);
                    array2 = null;
                }
            }
            catch (Exception)
    ...
    public static Image loadImage(string path)
    {
        ...
        path = Main.res + "/x" + mGraphics.assetZoomLevel + path;
        path = cutPng(path);
        try
        {
    ...
    public static Image loadImageResize(string path)
    {
        ...
        {
            result = Image.createImage(path);
            if (mGraphics.zoomLevel != mGraphics.assetZoomLevel)
            {
                resizeImage(result);
            }
        }
    ...
    {
        targetX = System.Math.Max(1, targetX);
        targetY = System.Math.Max(1, targetY);
        RenderTexture previous = RenderTexture.active;
        RenderTexture temporary = RenderTexture.GetTemporary(targetX, targetY, 0, RenderTextureFormat.ARGB32);
        try
        {
            Graphics.Blit(texture2D, temporary);
            RenderTexture.active = temporary;
            Texture2D resized = new Texture2D(targetX, targetY, TextureFormat.RGBA32, false);
            resized.ReadPixels(new Rect(0f, 0f, targetX, targetY), 0, 0);
            resized.Apply();
            return resized;
        }
        finally
    ...
    public static void resizeImage(Image img)
    {
        ...
        int w = System.Math.Max(1, img.w * mGraphics.zoomLevel / mGraphics.assetZoomLevel);
        int h = System.Math.Max(1, img.h * mGraphics.zoomLevel / mGraphics.assetZoomLevel);
    ...
    public static void updateCanvasSize()
    {
        ...
        hw = w / 2;
        hh = h / 2;
        isTouchControl = w >= 240;
        isTouchControlSmallScreen = w < 320;
        isTouchControlLargeScreen = w >= 320;
        GameScr.d = ((w <= h) ? h : w) + 20;
    }

    public static Image prepareAssetImage(Image img)
    {
        if (img != null && mGraphics.zoomLevel != mGraphics.assetZoomLevel)
        {
            resizeImage(img);
        }
        return img;
    ...
    public static string cutPng(string str)
    {
        ...
        {
            result = str.Replace(".gif", string.Empty);
        }
        else if (str.Contains(".jpeg"))
        {
            result = str.Replace(".jpeg", string.Empty);
        }
        else if (str.Contains(".jpg"))
        {
            result = str.Replace(".jpg", string.Empty);
        }
        return result;
    }
