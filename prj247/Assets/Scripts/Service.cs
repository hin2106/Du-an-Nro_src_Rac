...
public class Service
{
    ...
    public void charInfo(string day, string month, string year, string address, string cmnd, string username, string password)
    {
        Message message = null;
        try
        {
    ...
    public void useItem(sbyte type, sbyte where, sbyte index, short template)
    {
        ...
        if (type == 0 && where == 1 && GameScr.gI() != null)
        {
            ItemTemplate itemTemplate = null;
            if (index == -1)
            {
                itemTemplate = ItemTemplates.get(template);
            }
            else if (Char.myCharz().arrItemBag != null && index >= 0 && index < Char.myCharz().arrItemBag.Length && Char.myCharz().arrItemBag[index] != null)
            {
                itemTemplate = Char.myCharz().arrItemBag[index].template;
            }
            if (itemTemplate != null && itemTemplate.type == 6)
            {
                GameScr.gI().onUsePotionRequested();
            }
        }
        Message message = null;
        try
    ...
    public void setClientType()
    {
        ...
        try
        {
            ...
            message.writer().writeByte(mGraphics.assetZoomLevel);
    ...
    public void setClientType2()
    {
        ...
        try
        {
            ...
            message.writer().writeByte(mGraphics.assetZoomLevel);
            Res.outz("gui asset zoomlevel = " + mGraphics.assetZoomLevel);
            message.writer().writeBoolean(value: false);
            message.writer().writeInt(GameCanvas.w);
            message.writer().writeInt(GameCanvas.h);
            message.writer().writeBoolean(TField.isQwerty);
    ...
    public void requestLatencyProbe(long token)
    {
        Message message = null;
        try
        {
            message = new Message(75);
            message.writer().writeLong(token);
            session.sendMessage(message);
        }
        catch (Exception)
        {
