...
public class GameScr : mScreen, IChatable
{
    ...
    public static Image imgDragonPass;
    ...
    private long pendingUsePotion;

    private bool pendingUsePotionEffectPlayed;
    ...
    public static void loadBg()
    {
        ...
        if (GameCanvas.isTouch)
        {
            ...
            imgMail = GameCanvas.loadImage("/mainimage/mail.png");
            imgDragonPass = GameCanvas.loadImage("/mainimage/event.png");
        ...
        isAnalog = ((Rms.loadRMSInt("analog") != 0) ? 1 : 0);
        gamePad = new GamePad();
    ...
    public void onOSkill(sbyte[] oSkillID)
    {
        ...
        for (int i = 0; i < oSkillID.Length && i < onScreenSkill.Length; i++)
        {
    ...
    public void onKSkill(sbyte[] kSkillID)
    {
        ...
        for (int i = 0; i < kSkillID.Length && i < keySkill.Length; i++)
        {
            for (int j = 0; j < Char.myCharz().vSkillFight.size(); j++)
            {
                Skill skill = (Skill)Char.myCharz().vSkillFight.elementAt(j);
    ...
    public void doSetKeySkill(SkillTemplate skillTemplate)
    {
        ...
        MyVector myVector = new MyVector();
        for (int i = 0; i < keySkill.Length && i < array.Length; i++)
        {
    ...
    {
        if (Char.myCharz().myskill != null && ((Char.myCharz().myskill.template.manaUseType != 1 && Char.myCharz().cMP < (long)Char.myCharz().myskill.manaUse) || (Char.myCharz().myskill.template.manaUseType == 1 && Char.myCharz().cMP < Char.myCharz().cMPFull * (long)Char.myCharz().myskill.manaUse / 100.0)))
        {
    ...
    public override void updateKey()
    {
        ...
        checkAuto();
        string pendingHotkey = ModFunc.GI().ConsumePendingHotkey();
        if (ModFunc.GI().ExecuteHotkey(pendingHotkey, this))
        {
            GameCanvas.keyAsciiPress = 0;
            GameCanvas.clearKeyHold();
            GameCanvas.clearKeyPressed();
    ...
    private void checkClick()
    {
        ...
        ModFunc.HudPointerState requestPointerState = ModFunc.PushHudPointer(ModFunc.HUD_REQUEST);
        bool requestAccepted = popUpYesNo != null && popUpYesNo.cmdYes != null && popUpYesNo.cmdYes.isPointerPressInside();
        ModFunc.PopHudPointer(requestPointerState);
        if (requestAccepted)
        {
            popUpYesNo.cmdYes.performAction();
        }
    ...
    public void onUsePotionSuccess()
    {
        lastUsePotion = mSystem.currentTimeMillis();
        SoundMn.gI().eatPeans();
        if (pendingUsePotion > 0)
        {
            pendingUsePotionEffectPlayed = true;
        }
    }

    public void onUsePotionRequested()
    {
        pendingUsePotion = mSystem.currentTimeMillis();
        pendingUsePotionEffectPlayed = false;
    }

    public void confirmUsePotion()
    {
        long now = mSystem.currentTimeMillis();
        if (pendingUsePotion <= 0 || now - pendingUsePotion > 10000)
        {
            pendingUsePotion = 0;
            pendingUsePotionEffectPlayed = false;
            return;
        }
        if (!pendingUsePotionEffectPlayed)
        {
            onUsePotionSuccess();
        }
        pendingUsePotion = 0;
    ...
    private void UpdateKeyTouchControl()
    {
        ...
        if (ModFunc.GI().showCharsInMap)
        {
            ...
            for (int i = 0; i < chars.size(); i++)
            {
                ...
                if (ModFunc.IsHudPointerInRegisteredRow(ModFunc.HUD_PLAYERS, numY, 10) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                {
        ...
        if (ModFunc.notifBoss)
        {
            ...
            {
                if (ModFunc.IsHudPointerInRegisteredRow(ModFunc.HUD_BOSS, numX, 10) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                {
        ...
        if (GameCanvas.isTouchControl)
        {
            ...
            if (!TileMap.isOfflineMap() && ModFunc.IsHudImagePointerIn(ModFunc.HUD_CHAT, xC, yC + mGraphics.addYWhenOpenKeyBoard, 34, 34, 0, 0, 34, 34, imgChat))
            {
                mScreen.keyTouch = 15;
                GameCanvas.isPointerJustDown = false;
                isPointerDowning = false;
            ...
            updateGamePad();
            int beanTouchX = (!GameCanvas.isTouch || (gamePad.isSmallGamePad && isAnalog == 1)) ? xSkill + xHP - 1 : xHP;
            Image beanImage = (isAnalog != 0) ? imgHP3 : imgHP1;
            if (ModFunc.IsHudImagePointerIn(ModFunc.HUD_BEAN, beanTouchX, yHP - 1, 40, 40, 0, 0, 40, 40, beanImage) && Char.myCharz().statusMe != 14 && mobCapcha == null)
            {
                mScreen.keyTouch = 10;
                GameCanvas.isPointerJustDown = false;
                isPointerDowning = false;
        ...
        else if (isHaveSelectSkill)
        {
            ...
            ModFunc.HudPointerState skillPointerState = ModFunc.PushHudPointer(ModFunc.HUD_SKILLS);
            try
            {
            keyTouchSkill = -1;
            bool flag = false;
            int skillPerRow = (onScreenSkill.Length > 5) ? ((onScreenSkill.Length + 1) / 2) : onScreenSkill.Length;
            int secondRowStart = skillPerRow;
            if (onScreenSkill.Length > skillPerRow && (GameCanvas.isPointerHoldIn(xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, skillPerRow * wSkill, wSkill) || GameCanvas.isPointerHoldIn(xSkill + xS[secondRowStart] - wSkill / 2 + 12, yS[secondRowStart] - wSkill / 2 + 12, skillPerRow * wSkill, wSkill)))
            {
                flag = true;
            }
            if (flag || GameCanvas.isPointerHoldIn(xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, skillPerRow * wSkill, wSkill) || (!GameCanvas.isTouchControl && GameCanvas.isPointerHoldIn(xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, wSkill, onScreenSkill.Length * wSkill)))
            {
                GameCanvas.isPointerJustDown = false;
                isPointerDowning = false;
                ...
                if (num > onScreenSkill.Length - 1)
                {
                    num = onScreenSkill.Length - 1;
                }
                keyTouchSkill = num;
                if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                {
            ...
            {
                ModFunc.PopHudPointer(skillPointerState);
            }
        }
        if (GameCanvas.isPointerJustRelease)
        {
    ...
    public override void update()
    {
        ...
        try
        {
            ...
            for (int k = 0; k < vMob.size(); k++)
            {
                ...
                {
                    Cout.LogError("Loi update mob gamesc: " + ex.ToString());
                }
            }
    ...
    public override void paint(mGraphics g)
    {
        ...
        for (int num = 0; num < vMob.size(); num++)
        {
            try
            {
                Mob mob = (Mob)vMob.elementAt(num);
                mob?.paint(g);
            }
            catch (Exception ex)
            {
                Cout.LogError("Loi paint mob gamesc: " + ex.ToString());
            }
        }
        ...
        if (!isPaintOther)
        {
            ...
            resetTranslate(g);
            if (GameCanvas.isTouch && GameCanvas.isTouchControl && !DragonPass.DragonPassUI.Visible)
            {
                paintTouchControl(g);
            }
            resetTranslate(g);
            ...
            if (!DragonPass.DragonPassUI.Visible)
            {
                ...
                if (!ModFunc.IsMailPanelVisible())
                {
                    ...
                    {
                        ((ItemTime)textTime.elementAt(num25)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 35 : 104) + num25 * 12);
                    }
                }
            }
            paintXoSo(g);
            ...
            if (!yourNumber.Equals(string.Empty))
            {
                ...
                {
                    mFont.tahoma_7b_white.drawString(g, strPaint[num26], 5, 110 + num26 * 18, 0, mFont.tahoma_7b_dark);
                }
            }
        }
        int num27 = 0;
    ...
    {
        Mob mobById = findMobInMap((int)mobIndex);
        if (mobById != null)
        {
            return mobById;
        }
        int index = mobIndex;
        if (index < 0 || index >= vMob.size())
        {
            return null;
        }
        return (Mob)vMob.elementAt(index);
    }
    ...
    private void paintTouchControl(mGraphics g)
    {
        ...
        {
            int chatY = yC + mGraphics.addYWhenOpenKeyBoard;
            ModFunc.HudPaintState chatState = ModFunc.BeginHudElement(g, ModFunc.HUD_CHAT, "Nút chat", xC, chatY, 34, 34);
            ...
            ModFunc.EndHudElement(chatState);
        }
        _ = isUseTouch;
    }
 	...
 	private void paintSelectedSkill(mGraphics g)
	{
		...
		else
		{
			...
			int beanX = (!GameCanvas.isTouch || (gamePad.isSmallGamePad && isAnalog == 1)) ? xSkill + xHP - 1 : xHP;
			ModFunc.HudPaintState beanState = ModFunc.BeginHudElement(g, ModFunc.HUD_BEAN, "Nút dùng đậu", beanX, yHP - 1, 40, 40);
			if (!GameCanvas.isTouch)
			{
			...
			else if (Char.myCharz().statusMe != 14)
			{
				...
				{
                    int analogBarHeight = 18;
                    int analogFill = 0;
                    if (num2 < 10000)
                    {
                        analogFill = (int)(num2 * analogBarHeight / 10000);
                    }
					g.setColor(9670800);
                    g.fillRect(xHP + 7, yHP + 9, 20, analogBarHeight);
					g.setColor(16777215);
                    g.fillRect(xHP + 7, yHP + 9 + ((analogFill != 0) ? (analogBarHeight - analogFill) : 0), 20, (analogFill == 0) ? analogBarHeight : analogFill);
					g.drawImage((mScreen.keyTouch != 10) ? imgHP3 : imgHP4, xHP + 17, yHP + 17, mGraphics.HCENTER | mGraphics.VCENTER);
                    mFont.tahoma_7_red.drawString(g, string.Empty + hpPotion, xHP + 17, yHP + 12, 2);
				}
			}
			ModFunc.EndHudElement(beanState);
			if (isHaveSelectSkill)
			{
				...
				int skillMinX = GameCanvas.w;
				int skillMinY = GameCanvas.h;
				int skillMaxX = 0;
				int skillMaxY = 0;
				for (int skillIndex = 0; skillIndex < num4; skillIndex++)
				{
					skillMinX = System.Math.Min(skillMinX, xSkill + xS[skillIndex] - 2);
					skillMinY = System.Math.Min(skillMinY, yS[skillIndex] - 14);
					skillMaxX = System.Math.Max(skillMaxX, xSkill + xS[skillIndex] + 28);
					skillMaxY = System.Math.Max(skillMaxY, yS[skillIndex] + 30);
				}
				ModFunc.HudPaintState skillPaintState = ModFunc.BeginHudElement(g, ModFunc.HUD_SKILLS, "Các ô kỹ năng và phím tắt", skillMinX, skillMinY, System.Math.Max(1, skillMaxX - skillMinX), System.Math.Max(1, skillMaxY - skillMinY));
				for (int i = 0; i < num4; i++)
				{
					if (Main.isPC)
					{
                        if (isAnalog == 0 && mGraphics.addYWhenOpenKeyBoard <= 0 && !ChatTextField.gI().isShow)
                        {
                            int num5 = -13;
                            if (num4 > 4 && i < 4)
                            {
                            ...
                            UnityEngine.KeyCode skillKey = ModFunc.GI().GetHotkey(ModFunc.HOTKEY_SKILL_PREFIX + (i + 1));
                            string keyLabel = (skillKey == UnityEngine.KeyCode.None) ? "-" : ModFunc.GetHotkeyName(skillKey);
                            mFont.tahoma_7b_dark.drawString(g, keyLabel, xSkill + xS[i] + 14, yS[i] + num5, mFont.CENTER);
                            mFont.tahoma_7b_white.drawString(g, keyLabel, xSkill + xS[i] + 14, yS[i] + num5 + 1, mFont.CENTER);
                        }
					}
				...
				ModFunc.EndHudElement(skillPaintState);
			}
            paintGamePad(g);
            ...
            if (ModFunc.IsMailPanelVisible())
            {
                ModFunc.PaintMailPanel(g);
            }

            DragonPass.DragonPassUI.Paint(g);

            if (ModFunc.isShowMenuChat && !QuaNapTuan.isNapTuan)
            {
    ...
    public void onChatFromMe(string text, string to)
    {
        ...
        else if (ChatTextField.gI().strChat == "Sửa tài khoản" && text != string.Empty)
        {
            int editIndex = Rms.loadRMSInt("editAccIndex");
            if (editIndex >= 0 && editIndex < ModFunc.accounts.Count)
            {
                string[] parts = text.Split('$');
                if (parts.Length >= 2)
                {
                    string newUser = parts[0].Trim();
                    string newPass = parts[1].Trim();
                    if (!string.IsNullOrEmpty(newUser) && !string.IsNullOrEmpty(newPass))
                    {
                        ModFunc.GI().UpdateAccount(editIndex, newUser, newPass);
                        GameScr.info1.addInfo("Đã cập nhật tài khoản: " + newUser, 0);
                    }
                    else
                    {
                        GameScr.info1.addInfo("Tài khoản hoặc mật khẩu không hợp lệ", 0);
                    }
                }
                else
                {
                    GameScr.info1.addInfo("Định dạng: username$password", 0);
                }
                Rms.saveRMSInt("editAccIndex", -1);
                Rms.saveRMSInt("isEditingAcc", 0);
            }
            ChatTextField.gI().strChat = "Chat";
            ChatTextField.gI().tfChat.name = "chat";
            ChatTextField.gI().tfChat.setIputType(TField.INPUT_TYPE_ANY);
    ...
    private void updateGamePad()
    {
        ...
        if (ModFunc.IsHudImagePointerIn(ModFunc.HUD_FIRE, xF, yF, 40, 40, 0, 0, 40, 40, imgFire0))
        {
            mScreen.keyTouch = 5;
            if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
            {
        ...
        ModFunc.HudPointerState joystickPointerState = ModFunc.PushHudPointer(ModFunc.HUD_JOYSTICK);
        try
        {
            gamePad.update();
        }
        ...
        {
            ModFunc.PopHudPointer(joystickPointerState);
        }
        if (ModFunc.IsHudImagePointerIn(ModFunc.HUD_TARGET, xTG, yTG, 34, 34, 0, 0, 34, 34, imgFocus))
        {
            mScreen.keyTouch = 13;
            GameCanvas.isPointerJustDown = false;
            isPointerDowning = false;
    ...
    private void paintGamePad(mGraphics g)
    {
        if (isAnalog != 0 && Char.myCharz().statusMe != 14)
        {
            ModFunc.HudPaintState fireState = ModFunc.BeginHudElement(g, ModFunc.HUD_FIRE, "Nút đánh", xF, yF, 40, 40);
            g.drawImage((mScreen.keyTouch != 5 && mScreen.keyMouse != 5) ? imgFire0 : imgFire1, xF + 20, yF + 20, mGraphics.HCENTER | mGraphics.VCENTER);
            ModFunc.EndHudElement(fireState);
            int joystickY = gamePad.isLargeGamePad ? GameCanvas.h - 90 : GameCanvas.h - 135;
            ModFunc.HudPaintState joystickState = ModFunc.BeginHudElement(g, ModFunc.HUD_JOYSTICK, "Joy stick", 0, joystickY, 90, 90);
            gamePad.paint(g);
            ModFunc.EndHudElement(joystickState);
            ModFunc.HudPaintState targetState = ModFunc.BeginHudElement(g, ModFunc.HUD_TARGET, "Nút đổi mục tiêu", xTG, yTG, 34, 34);
            g.drawImage((mScreen.keyTouch != 13) ? imgFocus : imgFocus2, xTG + 17, yTG + 17, mGraphics.HCENTER | mGraphics.VCENTER);
            ModFunc.EndHudElement(targetState);
