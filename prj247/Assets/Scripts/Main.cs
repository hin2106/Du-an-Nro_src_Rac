...
public class Main : MonoBehaviour
{
	...
	private long renderedFrameWindowStart;
	...
	private void OnGUI()
	{
		...
		if (Event.current.type.Equals(EventType.Repaint) && paintCount <= updateCount)
		{
			...
			if (renderedFrameWindowStart == 0L)
			{
				renderedFrameWindowStart = now;
			}
			renderedFrameCount++;
			long elapsed = now - renderedFrameWindowStart;
			if (elapsed >= 1000L)
			{
				currentFps = (int)(renderedFrameCount * 1000L / elapsed);
	...
	{
		if (isPC && Rms.loadRMSInt("lastZoomlevel") != mGraphics.assetZoomLevel)
		{
			Rms.clearAll();
	...
	private void FixedUpdate()
	{
		...
		if (count >= 10)
		{
			...
			FastResourceDownloader.Instance.ProcessFileQueue();
			f++;
			if (f > 8)
			{
				f = 0;
	...
	private void checkInput()
	{
		...
		{
			bool handledHotkey = GameCanvas.panel != null && GameCanvas.panel.HandleHotkeyKeyDown(Event.current.keyCode);
			if (!handledHotkey)
			{
				handledHotkey = ModFunc.GI().TryQueueHotkey(Event.current.keyCode);
			}
			if (!handledHotkey)
			{
		...
		{
			if (!ModFunc.GI().IsConfiguredHotkey(Event.current.keyCode))
			{
