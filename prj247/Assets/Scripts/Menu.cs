public class Menu
{
	...
	{
		startAt(menuItems, 0);
		int totalMenuWidth = menuItems.size() * menuW;
		int paddingRight = 8;
		bool isRightPanel = x >= GameCanvas.w - Panel.WIDTH_PANEL - 5;
		if (isRightPanel)
		{

			menuX = x + Panel.WIDTH_PANEL - totalMenuWidth - paddingRight;
		}
		else
		{

			int panelWidth = Panel.WIDTH_PANEL;
			int popupWidth = 180;
			int popupStartX = x + (panelWidth - popupWidth) / 2;
			if (totalMenuWidth <= popupWidth)
			{
				menuX = popupStartX + (popupWidth - totalMenuWidth) / 2;
			}
			else
			{
				menuX = x + (panelWidth - totalMenuWidth) / 2;
				if (menuX < x)
				{
					menuX = x;
				}
