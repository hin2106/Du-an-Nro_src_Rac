...
public class mFont
{
	...
	// GUIStyle wraps native Unity resources. Creating one for every string on
	// every OnGUI pass makes panels grow memory continuously, so keep one style
	// per font/alignment instead.
	private GUIStyle measurementStyle;

	private readonly GUIStyle[] drawStyles = new GUIStyle[3];
	...
	public mFont(sbyte id)
	{
		...
		this.id = id;
		text = "FontSys/x" + mGraphics.assetZoomLevel + "/" + text;
	...
	public int getWidthExactOf(string s)
	{
		try
		{
			GUIStyle style = getMeasurementStyle();
			return (int)style.CalcSize(new GUIContent(s)).x / mGraphics.zoomLevel;
		}
		catch (Exception ex)
		{
	...
	public int getHeight()
	{
		...
		GUIStyle gUIStyle = getMeasurementStyle();
		try
		{
	...
	public void _drawString(mGraphics g, string st, int x0, int y0, int align)
	{
		...
		GUIStyle gUIStyle = getDrawStyle(styleIndex);
		float num = 0f;
		float num2 = 0f;
		switch (align)
		{
	...
	private GUIStyle getMeasurementStyle()
	{
		if (measurementStyle == null)
		{
			measurementStyle = createScaledStyle();
		}
		return measurementStyle;
	}

	private GUIStyle getDrawStyle(int styleIndex)
	{
		if (drawStyles[styleIndex] == null)
		{
			drawStyles[styleIndex] = createScaledStyle(new GUIStyle(GUI.skin.label));
		}
		return drawStyles[styleIndex];
	}

	private GUIStyle createScaledStyle(GUIStyle style = null)
	{
		style = style ?? new GUIStyle();
		style.font = myFont;
		if (myFont != null && mGraphics.zoomLevel != mGraphics.assetZoomLevel)
		{
			style.fontSize = System.Math.Max(1, myFont.fontSize * mGraphics.zoomLevel / mGraphics.assetZoomLevel);
		}
		return style;
	}
