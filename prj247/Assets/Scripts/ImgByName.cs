...
public class ImgByName
{
	...
	{
		string filename = mGraphics.assetZoomLevel + "ImgByName_" + nameImg;
		...
		try
		{
			...
			result.img = GameCanvas.prepareAssetImage(Image.createImage(array, 1, array.Length - 1));
			_ = result.img;
			return result;
		}
		catch (Exception)
	...
	{
		string text = mGraphics.assetZoomLevel + "ImgByName_" + nameImg;
